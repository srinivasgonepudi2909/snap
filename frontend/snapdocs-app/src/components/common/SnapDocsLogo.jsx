...
const SnapDocsLogo = ({ 
  ...
  const logoSize = sizeClasses[size] || sizeClasses.normal;
  const logoTextSize = textSizeClasses[size] || textSize;

  return (
    <Link to={linkTo} className={`flex items-center space-x-3 group cursor-pointer ${className}`}>
      <div className={`${logoSize} bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center relative overflow-hidden shadow-lg group-hover:scale-110 transition-all duration-300`}>
      ...
      {showText && (
        <span className={`${logoTextSize} font-bold text-white group-hover:scale-110 transition-all duration-300`}>
          SnapDocs
        </span>
      )}
    </Link>
